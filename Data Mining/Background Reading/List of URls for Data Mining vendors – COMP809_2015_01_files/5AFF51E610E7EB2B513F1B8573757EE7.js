...
Draggable.prototype = {
  ...
  updateDrag: function(event, pointer) {
    ...
    var scrollUpdate = {left:0, top:0};
    if (this.options.scroll)
    {
      if (this.options.scroll == window)
      {
        var winscroll = this._getWindowScrollTL(this.options.scroll);
        scrollUpdate.top=winscroll.top;
        scrollUpdate.left=winscroll.left;
      }
      else
      {
        scrollUpdate.top=this.options.scroll.scrollTop;
        scrollUpdate.left=this.options.scroll.scrollLeft;
      }
    }
    pointer[0] += scrollUpdate.left;
    pointer[1] += scrollUpdate.top;
    this.element.bb_scrollUpdate = scrollUpdate;
